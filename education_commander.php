...
// جلب اسم المستخدم الحالي لعرضه في العنوان
$current_user_id = $_SESSION['id'] ?? null;
$user_stmt = $con->prepare("SELECT firstname, lastname FROM users WHERE id = ?");
$user_stmt->bind_param("i", $current_user_id);
$user_stmt->execute();
$user_result = $user_stmt->get_result();
$user_row = $user_result->fetch_assoc();
$user_name = ($user_row ? $user_row['firstname'] . ' ' . $user_row['lastname'] : 'غير محدد');

// جلب طلبات الدورات التي تحتاج قرار من قائد التعليم (من جميع الأقسام)
$sql = "SELECT DISTINCT 
    ce.*, 
    ...
    u.firstname as decision_by_firstname, 
...
LEFT JOIN (
    ...
    ORDER BY 
        id DESC
    LIMIT 1
) cd ON cd.course_employee_id = ce.id
LEFT JOIN 
    users u ON cd.decision_by = u.id
WHERE 
    ce.current_stage = 'education_commander'
    AND (cd.decision = 'pending' OR cd.decision IS NULL)
ORDER BY 
    ce.created_at DESC;";
...
<head>
    ...
    <title>قرارات دورات الموظفين - قائد التعليم </title>
    <style>
        .table-warning {
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <form id="decisionForm">
                    ...
                    <input type="hidden" name="stage" value="education_commander">
                    ...
                    <div class="mb-3">
                        <label for="recommendation" class="form-label">التوصيات:</label>
